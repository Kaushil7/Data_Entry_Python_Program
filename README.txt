EASY PWD Data Entry (https://ams.emahapwd.com/ams/login.do entry) 
Useful for Contractors and Civil Engineers of Maharashtra state who want to automate data entry on state government website
As all the quantities and data to the website can be entered at once instead of manually copying each and every dimension and quantities

There is also source code provided in the folder which has two variant of codes, also included with instruction in them after running it.


INSTRUCTION 
Note Please enter proper row details in integer only

1. Start putting data which u want to enter in the website from A column in auto excel sheet
and dont change name of the worksheet for .exe file.

2.Run the .exe file to get .txt file

(Maintain continuity in the data which is entered in the excel sheet)
2.Copy the contents of the file of .txt file generated to javascript console also images are attached for better 
clarity.

Further Queries Contact kaushil.nagrale@gmail.com
